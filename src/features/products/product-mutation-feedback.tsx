...
import type { ProductMutationResult } from "./actions";

export function ProductMutationFeedback({ result }: { result: ProductMutationResult }) {
  const { t } = useT();
  if (!result.success && result.errorCode === "IDLE") return null;
  if (result.success) return <p role="status" className="flex items-center gap-2 text-sm font-semibold text-emerald-800"><CheckCircle2 className="h-4 w-4" aria-hidden />{t("sync.current")}</p>;

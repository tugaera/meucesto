    ...
    <dialog ref={dialogRef} onClose={onClose} className="m-auto max-h-[90vh] w-[calc(100%-2rem)] max-w-lg border border-[var(--line)] bg-white p-0 text-[var(--ink)] backdrop:bg-black/50">
      <form action={action} className="flex max-h-[90vh] flex-col">
        ...
        <div className="grid gap-4 overflow-y-auto p-5">
          ...
          <div className="grid grid-cols-2 gap-3"><Field label={t("shopping.price")} htmlFor="detail-price"><Input id="detail-price" name="price" inputMode="decimal" value={formValues.price} onChange={(event) => update("price", event.target.value)} required /></Field><Field label={t("shopping.quantity")} htmlFor="detail-quantity"><Input id="detail-quantity" name="quantity" inputMode="decimal" value={formValues.quantity} onChange={(event) => update("quantity", event.target.value)} required /></Field></div>
          <Field label={t("shopping.originalPrice")} htmlFor="detail-original-price"><Input id="detail-original-price" name="originalPrice" inputMode="decimal" value={formValues.originalPrice} onChange={(event) => update("originalPrice", event.target.value)} /></Field>
          {!existing ? <>

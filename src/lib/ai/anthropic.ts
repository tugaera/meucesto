...
export async function extractWithAnthropic(input: { apiKey: string; model: string; bytes: Uint8Array; mimeType: ReceiptMimeType; timeoutMs: number }): Promise<ProviderExtractionResult> {
  const client = new Anthropic({ apiKey: input.apiKey, maxRetries: 0, timeout: input.timeoutMs });
  try {
